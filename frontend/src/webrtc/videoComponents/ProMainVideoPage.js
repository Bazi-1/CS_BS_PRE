...
const ProMainVideoPage = ()=>{
    ...
    const callStatus = useSelector(state=>state.rootreducer.callStatus)
    const streams = useSelector(state=>state.rootreducer.streams)
...
useEffect(() => {
    const fetchMedia = async () => {
        ...
        try {
            ...
            // Update call status in Redux (OK, serializable)
            dispatch(updateCallStatus('haveMedia', true));

            // Store MediaStream in refs, NOT Redux
            localStreamRef.current = stream;

            // Create PeerConnection
            const { peerConnection, remoteStream } = await createPeerConnection(addIce);

            // Store remote stream and peerConnection in refs
    ...
    useEffect(()=>{
        ...
        const fetchDecodedToken = async()=>{
            const resp = await axios.post('http://localhost:3001/validate-link',{token});
