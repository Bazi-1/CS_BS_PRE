...
const sendMessage = async (email, subject, message, username) => {
  try {
    const result1 = `INSERT INTO public.contact (email, subject, message, username)
                    VALUES ($1, $2, $3, $4) RETURNING message_id, email, subject, message, username`;

    const result = await query(result1, [email, subject, message, username]);

    console.log(
      `result in sendmessage ${JSON.stringify(JSON.stringify(result))}`
    );
